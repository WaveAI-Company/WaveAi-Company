...
### Emenda à ADR-0047 (2026-08-23) — a trilha pseudonimizada dura **12 meses contados da pseudonimização**, e por isso ganha data
**Status:** Proposta (2026-08-23) — vira Aceita no merge. Complementa a ADR-0047; não altera nada do que ela decidiu.

**Contexto:** a ADR-0047 decidiu que a trilha de leitura sobrevive à exclusão da conta do ator, pseudonimizada, mas **não fixou por quanto tempo** — e prazo era exatamente o que Medical/72 §5 deixara em aberto. A Política de Privacidade 1.2 fechou o número: **até 12 meses**. Faltava dizer *contados de quando*, e a diferença não é detalhe.

**[FATO] apurado antes de decidir:** as três trilhas guardam a data do **evento**, e nada mais — não existe registro de quando a linha foi pseudonimizada ([services/auth.py:325](services/api/app/services/auth.py:325) grava só `actor_user_id=None` e `actor_pseudonym`). Também **não existe rotina agendada de limpeza** em lugar nenhum do serviço: o `unverified_account_ttl_days` que poderia parecer uma é reciclagem **sob demanda**, avaliada no cadastro quando alguém tenta usar o mesmo endereço ([services/auth.py:161](services/api/app/services/auth.py:161)).

**Decisão:**
1. **O prazo conta da pseudonimização, não do evento.** Contar da data do evento faria a evidência sumir **no instante da exclusão** para toda leitura com mais de um ano — o profissional que acompanhou alguém em 2026 encerraria a conta em 2028 e apagaria a trilha inteira ao sair. Isso é o buraco que a ADR-0047 existe para fechar, reaberto por outro caminho. O relógio começa quando o registro perde o dono.
2. **Coluna `pseudonymized_at` nas três trilhas**, preenchida na mesma transação que grava o pseudônimo. Sem ela, o item 1 é inexequível: não há de onde tirar a data.
3. **O expurgo apaga a linha inteira**, e não apenas o pseudônimo. "Mantidos por até 12 meses" é o que a Política diz; deixar a linha viva sem identificador seria continuar mantendo o registro, e o texto passaria a não descrever o produto.
4. **Rotina explícita, não gatilho de uso.** O expurgo é um comando próprio, feito para ser agendado, e não um efeito colateral de alguma requisição. Pendurá-lo numa exclusão de conta ou numa leitura de trilha faria o cumprimento do prazo depender de alguém aparecer — se ninguém excluísse conta por dois anos, nada seria apagado e o documento estaria mentindo em silêncio.
5. **Enquanto não houver agendador, a promessa não está cumprida.** O código e os testes desta fatia tornam o prazo **cumprível**; quem o torna **cumprido** é o agendamento, que só existe com a infraestrutura de produção. A Política 1.2 **não pode ir a público antes disso** — trava já registrada em `Documentation/15` e como `TODO(infra)` no próprio texto.

**O que se abre mão, explicitamente:** o titular que ficou **perde a evidência** de que houve leitura depois de 12 meses — some a contagem, a data e o "foi o mesmo ator". É o custo de minimizar dado, e é a leitura correta do que a Política promete; mas é perda real para quem ficou, e não deve ser apresentada como neutra. Abre-se mão também de retroatividade: linhas já pseudonimizadas antes desta migration não têm data verdadeira, e recebem a data da migration — conservador (o relógio começa depois, nunca antes), e falso se alguém as ler como data real. Hoje não existe nenhuma fora de teste.

**Alternativas consideradas:** (a) **contar da data do evento** — rejeitada pelo item 1; (b) **zerar o pseudônimo e manter a linha** (a trilha continuaria dizendo "houve leitura", sem "foi o mesmo ator") — preterida pelo item 3, embora preserve mais para o titular: contradiria o texto publicado, e mudar o texto exigiria nova versão dos Termos; (c) **expurgo sob demanda**, no molde da reciclagem de conta não verificada — rejeitada pelo item 4; (d) **não fixar prazo** e escrever "enquanto for necessário" na Política — segue sendo o plano B se o agendador não sair antes da publicação.

**Consequências:** **migration** com `pseudonymized_at` nas três trilhas; `excluir_conta` passa a preenchê-la; módulo de expurgo com o prazo configurável por ambiente e um comando executável, feito para virar job agendado na fatia de deploy. Não altera rotas, papéis, o `AnalysisEngine` nem o que qualquer tela mostra. Relaciona ADR-0037 (a trilha que isto encurta), ADR-0027 (prazo sem executor é afirmar o que não é verdade), ADR-0049 (onde o agendador vai viver) e Medical/72 §5.
...
## ADR-0050 — Foto de perfil: **opcional**, no Postgres, re-codificada no servidor, visível ao contraparte de um **CareLink ativo** e **não** auditada
**Status:** Proposta (2026-08-29) — vira Aceita no merge. Pedido do fundador.

**Contexto:** paciente e profissional só se conhecem por `display_name` no contexto do vínculo. Uma foto opcional aumenta a confiança na hora de aceitar um CareLink — é o objetivo da função. Sem visibilidade mútua ela é vazia de sentido (decisão do fundador, 2026-08-29). Foto de rosto é **dado pessoal**, e o Container Apps **não tem disco persistente** (o que sobe morre com a réplica): guardar, apagar e mostrar uma imagem toca quatro decisões estruturais de uma vez, e é por isso que precisa de ADR antes de código.

**Decisão:**
1. **Guardar no Postgres**, tabela própria `profile_photos` com `user_id` PK/FK → `users(id)` **ON DELETE CASCADE**, 1:1 com o usuário, guardando os bytes já normalizados e o `content_type`. Tabela separada (não coluna no profile) para o blob **não** ser arrastado a cada leitura de perfil. **Sem fornecedor novo** (respeita a emenda à ADR-0049: proibido serviço proprietário além do runtime e do agendador). Custo de cota irrelevante nesta fase: avatar de ~30 KB × milhares cabe folgado nos 0,5 GB do Neon.
2. **Re-codificar no servidor** (dependência nova: **Pillow**). Toda imagem recebida é aberta, **redimensionada** (lado máximo 512 px), re-**codificada para JPEG** e gravada **sem EXIF**. Três motivos, todos de peso: **privacidade** — foto de rosto carrega GPS no EXIF, e re-codificar remove; **segurança** — re-encodar mata polyglot/`<script>` embutido e, com limite de pixels do Pillow, barra *decompression bomb*; **previsibilidade** — a saída tem tamanho e formato conhecidos. Entrada aceita `image/jpeg|png|webp`, teto de 5 MB no upload; **SVG é recusado** (é executável). Pillow entra em `pyproject` e na imagem da API.
3. **Dado pessoal → Política + `TERMS_VERSION`.** A Política de Privacidade passa a listar a foto na seção "Que dados coletamos" (opcional, mostrada ao contraparte de um vínculo ativo, apagada com a conta). Isso é mudança material: sobe a `versao` da Política (1.2 → 1.3) e, por referência, o `TERMS_VERSION` do servidor (1.1 → 1.2, ADR-0048) — o que **força reaceite** de todos os usuários pelo 409. O ripple foi aceito pelo fundador em 2026-08-29. **Fora do escopo desta fatia, e permanecem registradas como lacunas:** o aviso "avisaremos no app" (que não existe) e o backfill de `accepted_terms_version` nulo.
4. **Exclusão sai de graça (ADR-0047).** Como a foto vive no Postgres com FK CASCADE, `excluir_conta` a apaga junto com o resto do titular — sem varredura manual, sem apagar objeto em serviço externo.
5. **Visibilidade mútua, gated por CareLink ativo.** O dono vê a própria; o contraparte de um vínculo **ativo** (médico↔paciente) vê a do outro. Servida por endpoint **autenticado**, gated pelo mesmo predicado do `display_name`; **sem URL pública**, sem link assinado.
6. **A foto NÃO é auditada pela ADR-0037.** É tratada como **metadado de identidade do vínculo, igual ao `display_name`** — que já é mostrado ao contraparte **sem** evento de auditoria. A ADR-0037 audita a leitura dos **dados de saúde/sessão** (Result, anotações), não a identidade exibida no contexto de um vínculo consentido e ativo. Auditar cada busca de avatar seria a mesma inconsistência que a emenda à ADR-0037 já rejeitou para "existência e contagem" de notas: metadado não audita. **Esta é a decisão a revisar** — se o fundador quiser auditar, é trocar antes de codar.

**O que se abre mão, explicitamente:** (a) o blob no Postgres **infla a linha e não escala** para muitos usuários com fotos grandes — aceitável nesta fase, revisível no comercial; (b) **Pillow** aumenta a imagem da API em alguns MB (irrelevante para o cold start, cujo gargalo medido é a alocação de nó, não o pull); (c) subir o `TERMS_VERSION` **por uma foto** força reaceite geral e reacende as duas lacunas de Termos deferidas.
...
## ADR-0051 — Build dos apps móveis: **EAS Build**, credenciais **gerenciadas com backup exportado** e **Play App Signing**; iOS bloqueado por conta paga
**Status:** Aceita (2026-08-30). Decisão do fundador (caminho B) depois de exposto o caminho local.
**Contexto:** a frente atual é a **Play Store** (fatia G), e ela exige um build Android assinado. O app é **Expo/CNG**: não há `android/` nem `ios/` versionados — as pastas nativas são **saída** de `app.json` + config plugins. Ele carrega **sete dependências nativas** (`react-native-ble-plx`, `react-native-bluetooth-classic`, `react-native-reanimated`, `react-native-svg`, `expo-image-picker`, `expo-secure-store`, `expo-speech`), o que já o tira do Expo Go desde a ADR-0040. A máquina do fundador é **Windows**; não há Mac. O `eas.json` e o `extra.eas.projectId` do `app.json` **já existiam** desde a captação Android (commit `7327f8c`), e as ADR-0040 §6 e ADR-0042 já pressupunham "o build EAS" — mas **nenhuma ADR decidiu isso**. Esta paga essa dívida: regulariza uma adoção que vinha acontecendo por inércia.

**[FATO — verificado em 2026-08-30 na documentação oficial]**
1. **EAS free:** 15 builds Android + 15 iOS por mês, **concorrência 1**, fila de baixa prioridade (90+ min de espera em pico), timeout de 45 min por build.
2. **Keystore no EAS:** é gerada e guardada nos servidores da Expo, e **pode ser baixada** (`eas credentials` → *Download credentials from EAS to `credentials.json`*). A própria doc manda: *"Under no circumstances should you check it into your repository."*
3. **Play App Signing:** apps novos são **inscritos automaticamente**. Quem assina o APK que chega ao usuário é o **Google**; o desenvolvedor guarda apenas a **upload key**. E — o ponto que muda o risco — *"If you lose your upload key or suspect that it was compromised, you are not locked out of your app"*: dá para pedir **reset** da upload key.
4. **iOS:** distribuição interna (ad hoc) exige **Apple Developer Program pago**, limita a **100 aparelhos por ano** e requer o **UDID de cada aparelho** registrado antes; adicionar um device novo exige rebuild ou re-assinatura.

**[FATO] O build local do Android não foi descartado por mérito.** Foi tentado nesta sessão: `expo prebuild` gerou o projeto, o SDK está completo (build-tools 36.1.0, platforms até android-36, NDK) e o Java é 21. O Gradle reprovou com `Unable to establish loopback connection` — e a causa **não** foi diagnosticada: testes isolados provaram que loopback funciona tanto em Python quanto em Java nesta máquina. Fica como caminho vivo, não como beco sem saída.

**Decisão:**
1. **EAS Build** é o serviço de build para Android e iOS. Perfil `preview` (APK) para teste em aparelho; `production` (AAB) para a loja.
2. **Credenciais gerenciadas pelo EAS**, com **backup exportado** pelo fundador via `eas credentials` e guardado **fora do repositório**, no gerenciador de senhas. O `.gitignore` cobre `credentials.json`, `*.jks` e `*.keystore` antes de qualquer arquivo existir.
3. **Play App Signing ligado** (é automático). A chave de distribuição é do Google; a nossa é a **upload key**, e ela é recuperável. Isto rebaixa a perda da chave de *catastrófica e irreversível* para *chata* — e é o que torna aceitável a chave viver em servidor de terceiro.
4. **`android/` e `ios/` não são versionados.** São saída do `prebuild`; versioná-los cria duas fontes da verdade, e a que manda passa a ser a pasta, silenciosamente.
5. **`eas.json` declara `EXPO_PUBLIC_API_URL=https://api.waveai.tec.br`** nos perfis que geram build instalável. Sem isso o build sai apontando para o fallback `http://localhost:8000` de [`src/auth/api.ts:16`](apps/wave-app/src/auth/api.ts) — um app que não fala com servidor nenhum.
6. **iOS fica bloqueado**, e isso é registrado como pendência com preço, não como falha técnica: **US$ 99/ano** de Apple Developer Program. O EAS remove a necessidade do Mac; **não** remove a conta paga. Enquanto ela não existir, a captação iOS segue o "código-no-escuro" que a ADR-0040 §6 já reconhecia.
7. **Segredos só via ambiente** (regra do projeto, sem exceção aqui): keystore, senhas e credenciais Apple nunca entram em repositório, compose, Dockerfile ou CI.

**Escopo em relação à emenda da ADR-0049.** Aquela emenda proíbe "serviço proprietário além do runtime de container e do agendador", e a disciplina que ela protege é **manter a saída barata**. O EAS **não é dependência de runtime**: se a Expo sumir amanhã, o app publicado continua funcionando nos aparelhos e o build volta a ser local (o projeto nativo se regenera com `expo prebuild`). O que ele adiciona é uma dependência no **caminho de release** e um segredo guardado por terceiro — este último já mitigado pelo backup e pelo Play App Signing. **A emenda continua valendo como escrita, para runtime; esta ADR declara que ferramenta de build está fora do escopo dela.**

**Alternativas consideradas:** (a) **Build local com keystore própria** — zero fornecedor novo e zero decisão a registrar, mas exige toolchain Android mantido na máquina, não entrega iOS de forma alguma, e esbarrou no erro de Gradle acima. Preterido **agora**, não descartado. (b) **Keystore própria injetada no EAS** (`credentials.json` + variáveis de ambiente) — mais controle, mas troca um risco pequeno (chave em servidor de terceiro, com backup e reset possível) por um maior (chave em uma máquina só, sem redundância). (c) **Esperar a conta Apple para decidir os dois juntos** — preterido: trava o Android por uma dependência que é só do iOS.

**Consequências:** o fundador passa a ter uma **conta Expo no caminho de release** e é quem executa `eas login` e dispara os builds (credenciais são dele, não minhas). O `eas.json` entra no diff com `env` por perfil. O `.gitignore` ganha `android/`, `ios/` e os padrões de keystore. A fila gratuita significa que um build pode demorar **90+ minutos** em horário de pico — planejar demonstração com folga. Os 15 builds/mês por plataforma são teto real: rebuilds de tentativa e erro consomem cota. **Não** toca API, banco, modelo de dados nem o `AnalysisEngine`. Abre a fatia G (ficha da loja, classificação de conteúdo, formulário de segurança de dados, teste fechado) e mantém aberta a pergunta do endereço público do desenvolvedor e do teste fechado prolongado para contas pessoais, que precisam ser confirmados no ato do cadastro. Relaciona ADR-0040 (build EAS já pressuposto), ADR-0042, ADR-0049 e sua emenda (escopo de "serviço proprietário"), ADR-0050 (permissão de galeria a declarar na ficha) e [Documentation/17](Documentation/17_Infra_Estado_Custos_e_Fila.md) §4-G.

## ADR-0052 — Captação com a tela apagada: **serviço em primeiro plano** no Android, e a sessão sai de dentro da tela
**Status:** Aceita (2026-08-30). Pedido do fundador depois de testar no aparelho; notificação permanente aceita explicitamente.
**Contexto:** o fundador testou no celular e relatou que **mudar de aba encerra a captação**. Não é impressão da interface: [`app/patient/live.tsx`](apps/wave-app/app/patient/live.tsx) tem `useEffect(() => () => descartarRef.current(), [])` — um cleanup de desmontagem que chama `parar()` e `deviceConnection.disconnect()`. **A sessão inteira vive dentro do componente da tela**, então sair dela destrói o socket, o cronômetro e o stream. Depois ele esclareceu o alvo real: captar com a **tela bloqueada**, como um app de música — não com o processo morto.

**[FATO — verificado em 2026-08-30 na documentação do Android]**
1. O tipo de serviço para manter conexão com aparelho externo é **`connectedDevice`**, e a partir do **Android 14 (API 34)** declarar o tipo é obrigatório.
2. Exige `FOREGROUND_SERVICE` **e** `FOREGROUND_SERVICE_CONNECTED_DEVICE` no manifesto; como pré-requisito de runtime basta uma de `BLUETOOTH_CONNECT`/`BLUETOOTH_SCAN` concedida — **as duas que o app já pede**.
3. **A notificação é obrigatória e o app não pode escondê-la:** *"Foreground services show a status bar notification, to make users aware that your app is performing a task in the foreground and is consuming system resources."* A doc ainda desaconselha tentar disfarçá-la com prioridade mínima.

**[FATO] A biblioteca do Android não resolve isso por nós.** A captação Android é **SPP/Bluetooth Classic** (`connection.ts`, ADR-0040), com `react-native-bluetooth-classic` — e essa biblioteca **não oferece serviço em primeiro plano** (varrido o `BluetoothModule.d.ts`: nenhuma menção a foreground/service/background). O `isBackgroundEnabled` que existe no `app.json` pertence ao `react-native-ble-plx`, que aqui só cobre o **iOS/BLE**, e hoje está `false`. Ou seja: o serviço é **código nativo nosso**, via config plugin — é a parte cara desta decisão, e a que eu **não consigo exercitar** (sem aparelho, sem build; mesma condição que a ADR-0040 §6 registrou para o iOS).

**Decisão — em duas partes, deliberadamente separadas:**
1. **A sessão sai da tela e vira estado da aplicação.** Um provedor de sessão de captação, acima das rotas, dono do stream, da conexão, do cronômetro e do relatório. A tela passa a **ler e comandar**, não a possuir. Isto sozinho já resolve **navegar dentro do app** sem perder a captação, é **JavaScript puro**, não pede permissão nova e **é verificável** nos gates e no dev client.
2. **Serviço em primeiro plano do tipo `connectedDevice`**, com as duas permissões acima e uma notificação que diz, sem eufemismo, que o WaveAI está captando — com ação de voltar à sessão. Só então a captação sobrevive à tela apagada.

**A ordem importa e não é negociável:** a parte 2 sem a 1 não tem o que preservar, porque o dono do estado morre junto com a tela.

**O que se abre mão, explicitamente:**
(a) **A notificação permanente é preço, não escolha** — quem capta vai vê-la o tempo todo, e ela não pode ser ocultada. O fundador aceitou depois de informado.
(b) **A ficha da Play Store passa a declarar e justificar** o uso de serviço em primeiro plano; isso entra na fatia G junto do formulário de segurança de dados.
(c) **Bateria**: captação contínua com rádio ligado passa a aparecer no consumo do sistema, atribuída ao WaveAI.
(d) **Processo morto continua sem captar.** Se a pessoa fecha o app pelo gerenciador, a conexão morre — nenhum serviço sobrevive a isso, e a tela não deve prometer o contrário (ADR-0027).
(e) **iOS fica de fora por ora.** Lá não existe serviço em primeiro plano: o equivalente é `UIBackgroundModes: bluetooth-central` com `isBackgroundEnabled: true` no `react-native-ble-plx` — mais simples, porém sem build iOS nenhum para exercitar (ADR-0051). Fica registrado como o caminho, não como feito.
...
### Emenda à ADR-0053 (2026-08-31) — a evidência é **nossa e replicada**; o veredito negativo **não pode culpar o sensor**; e a fatia é menor do que a ADR supôs
**Status:** Proposta (2026-08-31) — vira Aceita no merge. Corrige o item 6 da decisão, substitui a ressalva (b) e ajusta as Consequências. O resto da ADR-0053 segue valendo.

**1. A ressalva (b) estava errada por omissão.** A ADR-0053 registrou o falso-negativo como *"conhecimento de domínio, não medição nossa"*. **Há medição nossa, pré-registrada e replicada**, e ela é o primeiro experimento do projeto: [DataScience/33 §4b](DataScience/33_Signal_Fidelity_Report.md) fecha o **Exp. B** com alfa relativa OF>OA em **duas sessões independentes** — razão **1,74** (d=1,79) e **1,48** (d=1,57), ambas p≪0,001, com **todos** os blocos OF acima de **todos** os OA. A literatura de apoio está em [DataScience/31](DataScience/31_Signal_Fidelity_Study_Protocol.md): **Rogers et al., 2016** (ThinkGear/FP1, potências relativas comparáveis a EEG de laboratório) e **Johnstone et al., 2021** (dry single-channel, 182 crianças, alfa frontal reduzido de olhos abertos). O paradigma funciona **frontalmente, em canal único seco** — que é exatamente o nosso setup.

**2. O item 6 da decisão estava incompleto, e do jeito perigoso.** Ele mandava o veredito negativo *"sugerir conferir o contato do sensor"*. O nosso próprio dado desmente que essa seja a explicação: a sessão **d2** não mostrou contraste **com `poor_signal` ótimo** — contato não era o problema. A causa foi o **estado**: riso involuntário, que injeta **EMG facial** em FP1 e cuja excitação emocional **desincroniza o alfa**; os dois apagam o contraste. Mandar a pessoa procurar defeito no sensor a faria caçar um problema que não existe.

**Passa a valer:** quando o padrão não aparece, a tela diz que **não deu para constatar** e apresenta as **duas** causas prováveis, nesta ordem — **estado e movimento** (rir, falar, apertar a mandíbula, mexer o rosto, não conseguir relaxar) e **contato do sensor** —, sem escolher uma. Segue proibido afirmar defeito no equipamento ou particularidade da pessoa (item 6 original, que continua).

> **[FATO — DataScience/33 §4b]** d1 (neutro): OF 29,3% × OA 16,9%, razão 1,74, p=2,2e-11. d2b (neutro): 35,4% × 23,9%, razão 1,48, p=1,2e-09. **d2 (riso): 21,1% × 21,9%, razão 0,97, p=0,73** — sem contraste, com contato bom. O pré-registro pegou o falso e confirmou o real.
