...
3. **Assinatura via SSE (cookie):** endpoint **read-only Server-Sent Events**, autenticado pelo **cookie httpOnly** que o web já usa (sem token na URL — ADR-0021/0025). Emite eventos `features`/`esense` ao vivo e, no fim, `closed` (com ponteiro ao `Result` persistido) ou `ended`. **Web-first** (o `EventSource` nativo do navegador; a superfície do espectador é o web — coerente com P6-b). Espectar no **mobile** é extensão futura (polyfill de `EventSource` = dep JS, não nativa).
