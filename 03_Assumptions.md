...
|---|---|---|---|---|
| **H-SIG-01** | O EEG de **canal único (FP1)** do NeuroSky fornece sinal com fidelidade suficiente para gerar valor clínico real. | 🟡 | Estudo de fidelidade (Exp. B fechado: alfa OF/OA replicado; Exp. C/D pendentes). Ver updates abaixo. | **Existencial.** Compromete toda a proposta clínica. Ver risco R-01. |
...
## Atualização (2026-07-24) — H-SIG-01 (recoleta N=1, 2 dias: resultado MISTO, não replicou)
Recoleta executada (autocaptação do dev, [ADR-0028](05_Decisions.md)): 6 blocos intercalados OF/OA por sessão, **2 dias**, pipeline travado (§12). **Dia 1:** efeito de Berger forte e limpo — alfa relativa OF>OA, razão **1,74**, p=2,2e-11, d=1,79, todos os blocos OF acima dos OA. **Dia 2:** **sem contraste** (razão 0,97, p=0,73), com **deriva temporal** de alfa em ambas as condições. `poor_signal` ótimo nos dois dias (contato não é o problema). **O efeito não replicou** → o critério pré-registrado (detectável **e replicável**) **não foi atingido**. **H-SIG-01 permanece 🔴→🟡**: não avança (sem replicação) nem rebaixa (o Dia 1 é evidência positiva real). Sem p-hacking — o Dia 2 não é descartado. Detalhes e próximos passos em [DataScience/33 §4b](DataScience/33_Signal_Fidelity_Report.md).

## Atualização (2026-07-24, cont.) — H-SIG-01 → 🟡 (Exp. B fechado, replicado em estado controlado)
**Supera o update anterior.** O operador identificou que o Dia 2 fora feito em **estado não-alvo** (riso involuntário → EMG + desincronização de alfa) e refez a sessão em estado neutro (**d2b**). Resultado: **d2b replicou o Dia 1** — razão **1,48**, p=1,2e-9, d=1,57, todos os blocos OF acima dos OA. Assim, nas **duas sessões de estado controlado (d1, d2b)** o alfa OF>OA é **detectável e replicável** → **Exp. B atinge a sua parte do gate §12 e FECHA**. **H-SIG-01 sobe para 🟡.** O d2 **não é apagado**: fica documentado como sessão de estado violado (exclusão pós-hoc, com razão declarada; o §12 ganha emenda exigindo estado neutro). **Limites (não é 🟢):** N=1 sujeito; alfa OF/OA é o **piso**; sem EEG de referência; reatividade (Exp. C), artefatos (Exp. D) e valor clínico seguem não provados. Detalhes em [DataScience/33](DataScience/33_Signal_Fidelity_Report.md).
